...
extern ALLEGRO_TIMER *mouvementTimer;
...
void DisplayNameInput(int Screen_H,int Screen_W) {
	...
	al_set_window_title(display, "StarFall - NameInput");
    ...
    // Declare a BITMAP called image, setting it's initial value to nullptr
    image = al_load_bitmap("Name_Screen640Wx580H.png");
    if (!image) {
...
void DisplayGame(int Screen_H,int Screen_W) {
	...
	al_register_event_source(event_queue, al_get_display_event_source(display));
 	...
 	al_register_event_source(event_queue, al_get_timer_event_source(mouvementTimer));
...
void DisplayEnd(int Screen_H, int Screen_W) {
	// initialize display
	display = al_create_display(Screen_W, Screen_H);
	if (!display) {
    	al_show_native_message_box(display, "Error", "Error", "Failed to initialize display!",
...
void InitializeAllegro() {
	...
	mouvementTimer = al_create_timer(1.0 / 50);
}
